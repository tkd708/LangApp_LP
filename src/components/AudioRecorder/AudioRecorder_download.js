...
const AudioRecorder = () => {
    ...
    const downloadRecording = () => {
        ...
        reader.onloadend = function () {
            ...
            console.log( view )
            //const audioBlob = new Blob( blobRecorded.blobURL, { type: 'audio/wav' } )
            const audioBlob = new Blob( [ view ], { type: 'audio/wav' } )
            console.log( audioBlob )
            const myURL = window.URL || window.webkitURL
            const audioElement = myURL.createObjectURL( audioBlob ).replace( 'blob:', '' )
            setDownloadUrl( audioElement )
        }
        //const myURL = window.URL || window.webkitURL;
        //const url = myURL.createObjectURL( blobRecorded.blob ).replace( 'blob:', '' );
        //setDownloadUrl( url )
    }
    ...
    return (
        ...
        >
            ...
            <Button
                style={ { marginTop: '10px' } }
                cta={ 'Download!' } // from the template
