...
module.exports.handler = async function (event, context) {
  ...
  console.log('received report...', body); ///////////// Fetch the LINE user id from dynamoDB for push messages
  ...
  await docClient.put(paramsReport).promise().then(res => console.log('Uploading the conversation analysis to dynamoDB was successful...', res)).catch(err => console.log('Uploading the conversation analysis to dynamoDB failed...', err)); /////////////// Fetch the past records from dynamoDB by UserName
  ...
  const userRecords = await docClient.query(paramsDynamo).promise().then(data => data.Items).catch(err => console.log('Fetch records from dynamoDB failed...', err));
  ...
  const dataURLVocabSize = await s3.upload(paramsS3VocabSize).promise().then(data => {
    console.log("Full graph successfully uploaded to S3", data);
    return data.Location;
  }).catch(err => console.log("Full graph upload to S3 error", err)); ///////////////// Push message to LINE bot the analysis resutls
  ...
  await client.pushMessage(userLineId, message).then(res => console.log('report push message successful...', res)).catch(err => console.log('error in report push message...', err)); // Progress charts
  ...
  await client.pushMessage(userLineId, pushImage1, notificationDisabled = true).then(res => console.log('image 1 push message successful...', res)).catch(err => console.log('error in image 1 push message...', err));
  const pushImage2 = {
    'type': 'image',
    'originalContentUrl': dataURLWordsPerMinute,
    'previewImageUrl': dataURLWordsPerMinute
  };
  await client.pushMessage(userLineId, pushImage2, notificationDisabled = true).then(res => console.log('image 2 push message successful...', res)).catch(err => console.log('error in image 2 push message...', err));
  const pushImage3 = {
    'type': 'image',
    'originalContentUrl': dataURLVocabSize,
    'previewImageUrl': dataURLVocabSize
  };
  await client.pushMessage(userLineId, pushImage3, notificationDisabled = true).then(res => console.log('image 3 push message successful...', res)).catch(err => console.log('error in image 3 push message...', err)); //////////// Finish the api
